...
;; Spec (https://clojure.org/guides/spec): the header comes back exactly as
;; the token's first segment encodes it. The data specs are in
;; examples/clojure/src/jwt_example/specs.clj.
(s/fdef decode-jwt-header
  :args (s/cat :auth-header ::specs/auth-header)
  :ret ::specs/jose-header
  :fn (fn [{{:keys [auth-header]} :args ret :ret}]
        (= ret (specs/auth-header->jose-header auth-header))))
